...
class LocalQuestionGen_byentity(BaseQuestionGen):
    ...
    async def agenerate(
        ...
        **kwargs,
        ...
        for ent_with_rel in tqdm(useful_entities):
            ...
            question_history = [f"Find all the related text units for {ent_with_rel_name}. and the text units of entities in relationships of {related_relationships_text_source} and {related_relationships_text_target}, and the relationships of {related_relationships_text_source} and {related_relationships_text_target}."]
            ...
            try:
                system_prompt = self.system_prompt.format(
                    context_data=context_data, question_count=question_count,entity=ent_with_rel_name,related_relationships_text_source=related_relationships_text_source,
                    related_relationships_text_target=related_relationships_text_target
                )
                ...
                try:
                    if "questions" in response:
                        questions.extend(json.loads(response)["questions"])
                    else:
                        questions.append(json.loads(response))
                except Exception:
                    raise ValueError(f"Error in parsing response: {response}")
